export const addPlantGrowthlogAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/plant-growth-log", {
      method: "POST",
      headers: {
  ...
  export async function getPlantGrowthlogTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/plant-growth-log"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deletePlantGrowthlogAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/plant-growth-log/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
