export const addLocationHousingAPI = async (formData: FormData) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/location-housing", {
      method: "POST",
      headers: {
  ...
  export async function getLocationHousingTableData() {
    // Fake delay
    await new Promise((resolve) => setTimeout(resolve, 10));
    
  
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/location-housing"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteLocationHousingAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/location-housing/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  export const updateLocationHousingAPI = async (id: string, basicidentity: FormData) => {
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/location-housing/${id}`, {
      method: "PUT",
      headers: {
        // "Content-Type": "application/json",
