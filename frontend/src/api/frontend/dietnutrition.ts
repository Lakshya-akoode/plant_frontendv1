export const addDietNutritionAPI = async (formData: FormData) => {
  ...
  if (!token) {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/diet-nutrition", {
      method: "POST",
      headers: {
  ...
  export async function getDietNutritionTableData() {
    // Fake delay
    await new Promise((resolve) => setTimeout(resolve, 10));
    
  
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/diet-nutrition"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteDietNutritionAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/diet-nutrition/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  export const updateDietNutritionAPI = async (id: string, basicidentity: FormData) => {
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/diet-nutrition/${id}`, {
      method: "PUT",
      headers: {
        // "Content-Type": "application/json",
