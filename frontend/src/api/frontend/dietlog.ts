export const addDietlogAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/diet-log", {
      method: "POST",
      headers: {
  ...
  export async function getDietlogTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/diet-log"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteDietlogAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/diet-log/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
