export const addSocialSubstanceAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/social-consumer-substance-use", {
      method: "POST",
      headers: {
  ...
  export async function getSocialSubstanceTableData() {
    // Fake delay
    await new Promise((resolve) => setTimeout(resolve, 10));
    
  
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/social-consumer-substance-use"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteSocialSubstanceAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/social-consumer-substance-use/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  export const updateSocialSubstanceAPI = async (id: string, basicidentity: FormData) => {
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/social-consumer-substance-use/${id}`, {
      method: "PUT",
      headers: {
        // "Content-Type": "application/json",
