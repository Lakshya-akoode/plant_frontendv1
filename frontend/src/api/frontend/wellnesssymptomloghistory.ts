export const addWellnessSymptomLogHistoryAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/wellness-symptom-log-history", {
      method: "POST",
      headers: {
  ...
  export async function getWellnessSymptomLogHistoryTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/wellness-symptom-log-history"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteWellnessSymptomLogHistoryAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/wellness-symptom-log-history/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
