export const addLifestyleHabitsAPI = async (formData: FormData) => {
  ...
  const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/lifestyle-daily-habits", {
      method: "POST",
      headers: {
  ...
  export async function getLifestyleHabitsTableData() {
    // Fake delay
    await new Promise((resolve) => setTimeout(resolve, 10));
    
  
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/lifestyle-daily-habits"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteLifestyleHabitsAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/lifestyle-daily-habits/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  export const updateLifestyleHabitsAPI = async (id: string, basicidentity: FormData) => {
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/lifestyle-daily-habits/${id}`, {
      method: "PUT",
      headers: {
        // "Content-Type": "application/json",
