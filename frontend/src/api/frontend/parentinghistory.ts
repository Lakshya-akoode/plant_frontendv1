export const addParentinglogHistoryAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/parenting-family-log", {
      method: "POST",
      headers: {
  ...
  export async function getParentinglogHistoryTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/parenting-family-log"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteParentinglogHistoryAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/parenting-family-log/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
