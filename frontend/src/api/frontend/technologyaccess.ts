export const addTechnologyAccessAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/technology-access", {
      method: "POST",
      headers: {
  ...
  export async function getTechnologyAccessTableData() {
    // Fake delay
    await new Promise((resolve) => setTimeout(resolve, 10));
    
  
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/technology-access"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteTechnologyAccessAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/technology-access/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  export const updateTechnologyAccessAPI = async (id: string, basicidentity: FormData) => {
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/technology-access/${id}`, {
      method: "PUT",
      headers: {
        // "Content-Type": "application/json",
