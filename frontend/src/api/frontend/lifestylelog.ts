export const addLifestylelogAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/lifestyle-activity-log", {
      method: "POST",
      headers: {
  ...
  export async function getLifestylelogTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/lifestyle-activity-log"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteLifestylelogAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/lifestyle-activity-log/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
