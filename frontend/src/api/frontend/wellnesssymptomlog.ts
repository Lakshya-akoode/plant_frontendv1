export const addWellnessSymptomLogAPI = async (formData: FormData) => {
  const userData = JSON.parse(localStorage.getItem("user") || "{}");
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/wellness-symptom-log", {
      method: "POST",
      headers: {
  ...
  export async function getWellnessSymptomLogTableData() {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+"api/wellness-symptom-log"); // Replace with actual API endpoint
      if (!response.ok) {
  ...
  export const deleteWellnessSymptomLogAPI = async (id: string) => {
    ...
    const response = await fetch(process.env.NEXT_PUBLIC_FRONTEND_API_URL+`api/wellness-symptom-log/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
