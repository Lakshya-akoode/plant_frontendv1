...
import { getDietlogById, updateDietlogAPI } from "../../../api/dietlog";
import { getWellnessSymptomLogById, updateWellnessSymptomLogAPI } from "../../../api/wellnesssymptomlog";
import { getLifestylelogById, updateLifestylelogAPI } from "../../../api/lifestylelog";
import { getPlantGrowthlogById, updatePlantGrowthlogAPI } from "../../../api/plantgrowthlog";
import { getPlantExtractlogById, updatePlantExtractlogAPI } from "../../../api/plantextract";
import { getParentinglogById, updateParentinglogAPI } from "../../../api/parenting";
...
const LogsTab = ({ user }) => {
  ...
  // Store log IDs for update operations
  ...
  const loadDietLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, diet: apiData._id }));
        }
  ...
  const loadWellnessLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, wellness: apiData._id }));
        }
      ...
      setLogIds(prev => ({ ...prev, wellness: null }));
    }
  };
  ...
  const loadLifestyleLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, lifestyle: apiData._id }));
        }
      ...
      setLogIds(prev => ({ ...prev, lifestyle: null }));
    }
  };
  ...
  const loadPlantGrowthLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, 'plant-growth': apiData._id }));
        }
      ...
      setLogIds(prev => ({ ...prev, 'plant-growth': null }));
    }
  };
  ...
  const loadPlantExtractLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, 'plant-extract': apiData._id }));
        }
      ...
      setLogIds(prev => ({ ...prev, 'plant-extract': null }));
    }
  };
  ...
  const loadParentingLogData = async () => {
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        // Store the log ID for update operations
        if (apiData._id) {
          setLogIds(prev => ({ ...prev, parenting: apiData._id }));
        }
      ...
      setLogIds(prev => ({ ...prev, parenting: null }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds.diet) {
          toast.error('Diet log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updateDietlogAPI(logIds.diet, formDataToSend);
        if (response.status === 'success') {
          toast.success('Diet log updated successfully!');
        } else {
          toast.error('Failed to update diet log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating diet log:', error);
        toast.error(error.message || 'Failed to update diet log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleWellnessSubmit = async (e) => {
    ...
    if (validateWellnessForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds.wellness) {
          toast.error('Wellness symptom log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updateWellnessSymptomLogAPI(logIds.wellness, formDataToSend);
        if (response.status === 'success') {
          toast.success('Wellness symptom log updated successfully!');
        } else {
          toast.error('Failed to update wellness symptom log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating wellness symptom log:', error);
        toast.error(error.message || 'Failed to update wellness symptom log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleLifestyleSubmit = async (e) => {
    ...
    if (validateLifestyleForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds.lifestyle) {
          toast.error('Lifestyle activity log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updateLifestylelogAPI(logIds.lifestyle, formDataToSend);
        if (response.status === 'success') {
          toast.success('Lifestyle activity log updated successfully!');
        } else {
          toast.error('Failed to update lifestyle activity log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating lifestyle activity log:', error);
        toast.error(error.message || 'Failed to update lifestyle activity log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handlePlantGrowthSubmit = async (e) => {
    ...
    if (validatePlantGrowthForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds['plant-growth']) {
          toast.error('Plant growth log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updatePlantGrowthlogAPI(logIds['plant-growth'], formDataToSend);
        if (response.status === 'success') {
          toast.success('Plant growth log updated successfully!');
        } else {
          toast.error('Failed to update plant growth log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating plant growth log:', error);
        toast.error(error.message || 'Failed to update plant growth log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handlePlantExtractSubmit = async (e) => {
    ...
    if (validatePlantExtractForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds['plant-extract']) {
          toast.error('Plant extract use log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updatePlantExtractlogAPI(logIds['plant-extract'], formDataToSend);
        if (response.status === 'success') {
          toast.success('Plant extract use log updated successfully!');
        } else {
          toast.error('Failed to update plant extract use log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating plant extract use log:', error);
        toast.error(error.message || 'Failed to update plant extract use log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleParentingSubmit = async (e) => {
    ...
    if (validateParentingForm()) {
      ...
      try {
        ...
        // Only update if log exists
        if (!logIds.parenting) {
          toast.error('Parenting & family log not found. Cannot update.');
          setLoading(false);
          return;
        }
        ...
        // Call update API
        const response = await updateParentinglogAPI(logIds.parenting, formDataToSend);
        if (response.status === 'success') {
          toast.success('Parenting & family log updated successfully!');
        } else {
          toast.error('Failed to update parenting & family log. Please try again.');
        }
      } catch (error) {
        console.error('Error updating parenting & family log:', error);
        toast.error(error.message || 'Failed to update parenting & family log. Please try again.');
      } finally {
        setLoading(false);
      }
    }
